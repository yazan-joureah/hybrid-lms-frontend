...
import { useInstructorAI } from '../../hooks/ai/useInstructorAI'
import type { InstructorAIOption } from '../../services/aiService'

const SUGGESTED: Record<InstructorAIOption, string[]> = {
    content_suggestions: [
        'اقترح تحسينات على ترتيب الوحدات',
        'أعطني أفكاراً لأسئلة مراجعة نهاية كل وحدة',
        'كيف أزيد من تفاعل الطلاب بهذا الكورس؟',
    ],
    performance_summary: [
        'قدّم ملخصاً عاماً لأداء الطلاب',
        'ركّز على نسبة الحضور',
        'ركّز على معدل إكمال الوحدات',
    ],
}

const MODE_LABELS: Record<InstructorAIOption, string> = {
...
export default function InstructorAIAssistant() {
    ...
    if (courses.length === 0) {
        return (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: 'calc(100vh - var(--header-height))', color: 'rgba(255,255,255,0.5)', gap: 10 }}>
                ...
                <div>أنشئ كورساً واحداً على الأقل لتتمكن من استخدام المساعد الذكي.</div>
            </div>
        )
    }
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - var(--header-height))', overflow: 'hidden' }}>
            {/* Header */}
            <div style={{ padding: '16px 28px', background: 'rgba(8,3,32,0.5)', backdropFilter: 'blur(10px)', borderBottom: '1px solid rgba(255,255,255,0.07)', display: 'flex', gap: 14, alignItems: 'center', flexWrap: 'wrap' }}>
                <div style={{ width: 44, height: 44, borderRadius: '50%', background: 'linear-gradient(135deg, #7c3aed, #a855f7)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, flexShrink: 0 }}>🤖</div>
                <div style={{ flex: '1 1 200px', minWidth: 0 }}>
                    <div style={{ fontSize: 16, fontWeight: 700 }}>المساعد الذكي للمحاضر</div>
                ...
                <div className="tab-bar" style={{ flexShrink: 0 }}>
                    {(Object.keys(MODE_LABELS) as InstructorAIOption[]).map(m => (
                        <div
                            key={m}
            ...
            <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
                ...
                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                    <div style={{ flex: 1, overflowY: 'auto', padding: '20px 28px', display: 'flex', flexDirection: 'column', gap: 18 }}>
                        {messages.length === 0 ? (
                            <div style={{ color: 'rgba(255,255,255,0.4)', textAlign: 'center', marginTop: 40 }}>
                                لا توجد محادثة بعد. اختر النوع من الأعلى واكتب طلبك بالأسفل للبدء 👋
                            </div>
                        ) : (
                            messages.map(m => (
                    ...
                    {/* Suggested prompts — حسب mode الحالي */}
                        ...
                        {SUGGESTED[mode].map(s => (
                            <button
                                ...
                                style={{
                                    ...
                                    color: '#c4b5fd', cursor: busy ? 'not-allowed' : 'pointer', fontFamily: 'inherit',
                    ...
                    <div style={{ padding: '14px 28px 20px', display: 'flex', gap: 10 }}>
                        <input
                            ...
                            placeholder={mode === 'content_suggestions' ? 'اسأل عن تحسين محتوى الكورس...' : 'اسأل عن أداء الطلاب...'}
                            disabled={busy}
                        ...
                        <button
                            ...
                            style={{
                                ...
                                cursor: (busy || !input.trim()) ? 'not-allowed' : 'pointer', opacity: (busy || !input.trim()) ? 0.6 : 1,
                ...
                <div style={{ width: 240, background: 'rgba(12,4,45,0.5)', borderRight: '1px solid rgba(255,255,255,0.07)', padding: '16px', overflowY: 'auto', direction: 'rtl' }}>
                    ...
                    {courses.map(c => {
                        ...
                        return (
                            <div
                                key={c._id}
                                onClick={() => !busy && setSelectedCourseId(c._id)}
                                style={{
                                    padding: '9px 12px', borderRadius: 10, fontSize: 13, cursor: busy ? 'not-allowed' : 'pointer',
